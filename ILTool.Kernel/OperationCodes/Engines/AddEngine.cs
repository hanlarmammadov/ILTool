...
{
    public class AddEngine : BinaryArithmeticEngine, IILOperationEngine
    {
        public void Execute(MethodContext context, MethodState state, object operand = null)
        {
            if (context.EvalStack.Count < 2)
                throw new InvalidStackSizeException("AddEngine: Evaluation stack should contain two elements to be added");
            ...
            if (!slot1.TypeToken.IsPrimitive || !slot2.TypeToken.IsPrimitive)
                throw new OperandsNotSupportedByOperationException(ByteCode.Add, slot1, slot2);

            ESSlot result = Compute(slot1, slot2, BinaryPrimitiveOpType.Add);
